...
The platform organises content into topic roadmaps with prerequisite links, lets users submit and rate resources, and ranks materials by relevance to each user based on their ratings and engagement history. Technically, it is a React frontend talking to a FastAPI backend over a REST API, with a PostgreSQL database. The ranking uses a basic user-based collaborative filtering approach with cosine similarity and a Bayesian average to handle sparse ratings. For new users with no history, an onboarding quiz seeds the cold-start fallback. When marking a topic complete, users take a short Gemini-generated quiz; getting three or more wrong sends them back to review first.
...
Recommender systems have been studied since the mid-1990s, starting with the GroupLens project \cite{resnick1994}, which applied collaborative filtering to Usenet articles. Collaborative filtering has since become a standard approach for personalised ranking \cite{koren2009}, used at scale by platforms like Netflix and Spotify. Opic applies a simplified version of these ideas -- not at the scale or sophistication of those systems, but enough to rank resources differently for different users based on their rating behaviour.

Opic was built to address this gap. The name comes from \textit{topic}, the core unit of learning in the system. It is a web-based platform that organises content into structured topic roadmaps, lets users submit and rate resources, and ranks materials by relevance to each person based on their ratings and engagement history.
...
These are addressed through topic roadmaps with prerequisite links, a community submission and admin review pipeline, and a basic user-based collaborative filtering approach for ranking resources. The CF approach was chosen over content-based filtering (which needs expert tagging) and matrix factorisation (which requires offline training) because it is simpler, interpretable, and works directly on live rating data \cite{herlocker2004}. The cold-start problem for new users with no rating history is partially mitigated through an onboarding quiz \cite{schein2002}.
...
\begin{itemize}
  ...
  \item A personalised ranking feature that suggests resources from day one (via the onboarding quiz cold-start) and produces different results for different users as they rate and complete resources.
...
\begin{itemize}
  ...
  \item \textbf{Star ratings and engagement tracking} -- explicit ratings combined with implicit signals (time spent, completion, revisits) feed into the ranking score \cite{hu2008}.
...
May (Weeks 3--4) & Engagement tracking, personalised ranking feature \\
...
\begin{enumerate}
  \item \textbf{Automated tests} using \texttt{pytest} and the FastAPI test client. These cover the auth flows and the ranking algorithm (scoring formula and cold-start fallback).
...
\begin{figure}[H]
\centering
\IfFileExists{arch.png}{\includegraphics[width=0.85\textwidth]{arch.png}}{\missingfig{arch.png}{System Architecture Diagram}}
...
\begin{itemize}
  ...
  \item \texttt{app/services/} -- business logic (personalised ranking)
...
The \texttt{ratings} table records each user's explicit feedback on a resource. It feeds directly into the user-resource matrix used for personalised ranking.
...
The \texttt{engagements} table captures implicit feedback signals that supplement the explicit star ratings. These signals are combined with the star rating to produce the weighted engagement score used for ranking (see Chapter 6).
...
The personalised ranking feature applies a basic collaborative filtering approach to order resources by predicted relevance for each user. It handles two cases: users who have rated resources, and new users with no history.
...
\subsection{User-Based Collaborative Filtering Approach}
...
\end{tikzpicture}
\caption{User-Resource Matrix. User C shares high scores on Resource 1 and 4 with User B. The system estimates User C's score for Resource 5 based on User B's engagement.}
...
This chapter has shown how each use case is realised through frontend pages, REST API endpoints, and database operations. The most technically involved parts are the personalised ranking feature and the AI knowledge quiz. The ranking requires building a user-resource matrix and computing cosine similarity; the quiz integrates an external generative AI API with a local fallback. The next chapter lists the tools used.
...
Figure~\ref{fig:ui-suggested} shows the personalised resource suggestions section on the topic detail page. Resources are ranked by their computed score and displayed below the topic's own resource list.
...
The part I found most interesting to build was the ranking feature. Getting anything useful out of a sparse rating matrix -- and having it still work for brand-new users with no history -- required combining a basic collaborative filtering approach with a Bayesian popularity score and an onboarding quiz fallback. It is a simple implementation, not a production-grade system, but it does produce noticeably different suggestions for different users, which was the goal.

...
The amount of learning content available online today is almost overwhelming. Platforms like YouTube, Coursera, and Medium publish thousands of articles, videos, and tutorials every day. For a student trying to learn something new -- say, data structures or web development -- the real problem is not finding content; it is figuring out \textit{what to learn, in what order, and what is actually worth their time}.
...
The system was built with university students in mind, particularly those studying Information and Communication Technology (ICT), Data Science (DS), and Business Information Technology (BIT), where self-directed learning is common and the volume of online material can be disorienting.
...
These three challenges are addressed respectively through structured topic roadmaps, a community-driven rating and admin-review pipeline, and a user-based collaborative filtering recommendation engine.
...
\begin{itemize}
  ...
  \item A JWT-based authentication system with a secure password reset flow, protecting all state-mutating endpoints behind role-aware access control.
...
\begin{itemize}
  \item \textbf{AI-generated knowledge quiz} -- drove the design of the \texttt{GET /topics/\{id\}/quiz} endpoint, the Gemini API integration, and the \texttt{QuizModal} component on the topic detail page.
  ...
  \item \textbf{Structured topic roadmaps with prerequisite relationships} -- drove the design of the \texttt{topics} and \texttt{topic\_prerequisites} tables and the topic detail page layout.
  \item \textbf{Community resource submission} -- drove the resource submission form and the \texttt{resources} table with a \texttt{status} field to gate visibility.
  ...
  \item \textbf{Star ratings and engagement tracking} -- drove the \texttt{ratings} and \texttt{engagements} tables and the engagement-score formula used in the recommender.
  \item \textbf{Personalised recommendations using collaborative filtering} -- drove the design of the recommender service, including the user-resource matrix, cosine similarity computation, and Bayesian average popularity score.
  \item \textbf{Cold-start onboarding quiz} -- drove the \texttt{onboarding\_questions} and \texttt{onboarding\_answers} tables and the topic-boosting logic in the cold-start fallback.
  \item \textbf{Progress tracking per topic} -- drove the \texttt{user\_progress} table and the progress page with the 30-day activity heatmap.
...
Figure~\ref{fig:ucd-overall} shows the complete use case diagram for the Opic system, illustrating the interactions between the two actors and all ten use cases.

\begin{figure}[H]
\centering
\IfFileExists{ucd.png}{\includegraphics[width=0.6\textwidth]{ucd.png}}{\missingfig{ucd.png}{Overall Use Case Diagram}}
\caption{Overall Use Case Diagram}
\label{fig:ucd-overall}
\end{figure}

The diagram highlights that the Learner role is the primary actor, interacting with the majority of use cases, while the Admin role is responsible for content governance and user management.
...
For users who have rated at least one resource, the system builds a user-resource matrix where each cell represents a weighted engagement score that combines explicit ratings with implicit engagement signals:
...
\caption{Home Feed -- Personalised Recommendations (FR-11, FR-14)}
\label{fig:ui-home}
\end{figure}
...
\caption{Resource Submission Form (FR-06)}
\label{fig:ui-submit}
\end{figure}
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item SQLAlchemy Documentation. (2024). \textit{SQLAlchemy}. Retrieved from \url{https://docs.sqlalchemy.org}

  \item Herlocker, J. L., Konstan, J. A., Terveen, L. G., \& Riedl, J. T. (2004). Evaluating collaborative filtering recommender systems. \textit{ACM Transactions on Information Systems}, 22(1), 5--53.
  ...
  \item JSON Web Tokens. (2024). \textit{JWT.io -- Introduction}. Retrieved from \url{https://jwt.io/introduction}

  \item OWASP Foundation. (2024). \textit{Password Storage Cheat Sheet}. Retrieved from \url{https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html}

  \item MDN Web Docs. (2024). \textit{Cross-Origin Resource Sharing (CORS)}. Retrieved from \url{https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS}
\end{enumerate}
